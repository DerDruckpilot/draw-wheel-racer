...
<p class="tilt-copy">Links / rechts kippen: Gewicht verlagern.<br>Vor / zurück kippen: lenken.</p><button class="sensor-button" id="gyro-enable">Neigung aktivieren</button><div class="sensor-actions"><button id="gyro-calibrate" disabled>Neutralstellung</button><button id="gyro-disable" disabled>Aus</button></div><p id="gyro-status" role="status">Aus · auch mit Touch spielbar</p>
...
for(const [axle,name] of ['rear','front'].entries()){
  const input=$<HTMLInputElement>('stiffness-'+name);input.oninput=()=>{if(sim)sim.stiffness[axle]=+input.value;input.setAttribute('aria-valuetext',+input.value<.3?'Weich':+input.value>.8?'Starr':'Elastisch');};input.addEventListener('keydown',e=>e.stopPropagation());
}
...
function loadLevel(id:number,home=true,discardRun=false){
  ...
  if(resumed)for(const [i,pad] of pads.entries()){pad.shape=sim.shapes[i];pad.draft=[];pad.render();$<HTMLInputElement>('stiffness-'+['rear','front'][i]).value=String(sim.stiffness[i]);}
  else for(const [i,name] of ['rear','front'].entries())sim.stiffness[i]=+$<HTMLInputElement>('stiffness-'+name).value;
...
async function boot(){
  try{
    ...
    if(import.meta.env.DEV||new URL(location.href).searchParams.has('test'))(window as any).__FORMDRIVE__={
      ...
      tune:(rear:number,front:number)=>{sim.stiffness=[clamp(rear,0,1),clamp(front,0,1)];},
