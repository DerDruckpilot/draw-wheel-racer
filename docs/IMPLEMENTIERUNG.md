...
| Radprofil | `src/wheel-profile.ts` | Gemeinsame Achsbreite und abgerundete Extrusion für Grafik und Kollision |
| Darstellung | `src/world-renderer.ts` | Instanzen, Kamera, Schatten, importierte Objekte und Effekte |
| Kamera | `src/chase-camera.ts` | Gefiltertes Folgen, Kriechfahrt-Totzone und allmähliche Rückkehr nach Verdeckungen |
| Bodendetails | `src/ground-dressing.ts` | Deterministisch gestreamte Pflanzen- und Kiesgruppen |
...
Die Kamera folgt einer gefilterten Fahrzeugposition und -richtung. Beim langsamen Rangieren unterdrückt eine Winkeltotzone kleine Lenkkorrekturen; kurze vertikale Stöße werden gedämpft. Nahezu senkrechte Fahrzeuglagen drehen die Ansicht nicht unkontrolliert. Ein Finger auf der freien Welt verändert den horizontalen Winkel um volle 360 Grad und die Höhe des Blicks ohne diese Verzögerung. Der gewählte Winkel bleibt relativ zur Fahrtrichtung erhalten; beim Gebietswechsel wird er zurückgesetzt. Pedale, Regler und Zeichenfelder besitzen unabhängige Pointer-Captures. Räumliche Kugelabfragen kürzen den Kameraarm vor Hindernissen; nach Freigabe fährt er langsam wieder aus. Die Kamera springt dabei nicht mehr zwischen seitlichen Ausweichpositionen. Niedrige Durchgänge verwenden eine flachere Position mit verzögertem Rückwechsel. Nahe Blätter zwischen Fahrzeug und Kamera werden ausgeblendet; feste Stämme bleiben sichtbar.
...
Innen- und Außenräder verwenden Ackermann-Lenkwinkel und unterschiedliche Soll-Drehzahlen entsprechend ihrem Kurvenradius. Unterschiedlich große Vorder- und Hinterräder erhalten ebenfalls angepasste Drehzahlen. Das verhindert, dass starre gleiche Motordrehzahlen die Kurve auseinanderdrücken. Der Schwerpunkt liegt tiefer; die Rollträgheit ist höher. Zusätzliche Rollgeschwindigkeitsdämpfung wirkt nur bei mindestens zwei belasteten Radkontakten. Sie setzt keinen Aufrichtwinkel und wirkt nicht in der Luft. Schwere Schräglagen können weiterhin zum Umkippen führen.

Gezeichnete Striche werden als abgerundete konvexe Extrusionen dargestellt und kollidiert: 0,42 Einheiten Achsbreite statt zuvor 0,19, bei unverändertem Radius von 0,095 in der Zeichenebene. Ein gemeinsames Profil liefert geschlossene Kollisionskörper und die Schultern der Grafik. Dünne Nabenverbindungen bleiben schmal. Exakt übermalte Abschnitte werden dedupliziert. Ein Polygonverbund bestimmt Volumen, Schwerpunkt und Trägheit, damit Überlappungen nicht mehrfach zählen. Die zusätzliche Breite fließt in Masse, Querträgheit, Wasservolumen und Fahrspuren ein. Gezeichnete Formen bleiben offen; Löcher werden nicht als gefüllte Scheiben behandelt.
...
Die äußeren Regler skalieren montierte Räder zwischen 0,4 und 1,4. Originalkonturen bleiben unverändert; Darstellung und Kollisionsprofile einschließlich Gummidicke verwenden denselben Faktor. Masse skaliert mit s³, Trägheit mit s⁵ und Wasservolumen mit s³. Veränderungen erfolgen während der Fahrt in kleinen physischen Schritten ohne Lagekorrektur des Chassis. Kontakte und Grafikpuffer werden wiederverwendet. Die alte Verformungssteuerung entfällt. Neigung wird in Bildschirmkoordinaten projiziert: rechts/links lenkt; vor/zurück bewegt den Schwerpunkt. Die Neutralstellung und beide Querformatausrichtungen bleiben unterstützt.
...
Zusätzliche Vegetation und Kies entstehen ab 2.2 in acht Einheiten großen Kacheln rund um das Fahrzeug. Ein separater Seed erhält ihre Plätze beim Zurückfahren und verändert keine Rätsel-IDs. Importierte Grasbüschel, Rooibos-Sträucher, blühende Gazanien, Farne und Kiesel folgen Bodenhöhe, Hangneigung, Biom und unregelmäßigen Bewuchsgruppen. Druckplatten und versunkene Stellen bleiben frei. Kacheln werden schrittweise aufgebaut, außerhalb der Nähe verworfen und für sichtbare Modellarten gemeinsam instanziert. Die Qualitätseinstellung begrenzt ihre Reichweite; ein entfernungsabhängiges Ausblenden mildert die Sichtgrenze. Kleine Details besitzen keine Collider und keinen eigenen Schattenwurf. Große sichtbare Steine bleiben physisch.
