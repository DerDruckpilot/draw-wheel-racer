...
for (const id of ids.length ? ids : Array.from({ length: EXPEDITION_COUNT }, (_, i) => i)) {
  ...
  for (let step = 0; step < 120 * (id === 12 ? 1400 : 360) && !car.finished; step++) {
    if (step % 15 === 0) {
      ...
      const advanced=id>=16||ADVENTURE_FEATURES.includes(zoneAt(sim.course,p.x)?.feature as any);
  ...
  console.log(JSON.stringify({ id, name: sim.course.name, finished: car.finished, time: +sim.elapsed.toFixed(1), x: +car.body.translation().x.toFixed(1), y: +car.body.translation().y.toFixed(1), pitch: +car.body.rotation().toFixed(2), zone: zoneAt(sim.course, car.body.translation().x)?.label, rescues: car.resets, mastered:sim.mechanics.mastered.length,freight:sim.mechanics.cargo?.health }));
