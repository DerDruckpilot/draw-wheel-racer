...
for (let id = 0; id < 13; id++) {
  ...
  for (let step = 0; step < 120 * 200 && !sim.cars[0].finished; step++) {
    ...
    if (step % 120 === 0) {
      const zone = zoneAt(sim.course, p.x + 2);
      const name: ShapeName = zone?.kind === 'lake' && p.x < zone.end - 7 ? 'paddle' : 'round';
      if (name !== last && step - lastChange > 240) { sim.requestShape(preset(name)); last = name; lastChange = step; }
    }
    sim.tick();
  }
