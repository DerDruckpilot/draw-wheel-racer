...
import {hooks} from './expedition-driver';
await initPhysics();
let failures=0;
for(const [name,shape] of [['large-ring',preset('round').map(p=>({x:p.x*1.2/.82,y:p.y*1.2/.82}))],['open-hooks',hooks],['small-ring',preset('compact')]] as const){
 ...
 for(let id=0;id<EXPEDITION_COUNT;id++){
  ...
  for(let i=0;i<120*240&&!v.finished;i++){
   sim.tick();if(v.body.translation().x>far+.4){far=v.body.translation().x;lastProgress=i;}
   if(i-lastProgress>120*16)break;
  }
  results.push({id,finished:v.finished,progress:Math.round(v.body.translation().x/sim.course.length*100),blocked:zoneAt(sim.course,v.body.translation().x)?.feature});
