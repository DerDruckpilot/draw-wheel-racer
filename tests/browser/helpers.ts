...
export async function drawStroke(page: Page, axle: 'rear' | 'front', points: [number,number][]) {
  const r = (await page.locator(`#drawing-${axle}`).boundingBox())!, scale=Math.min(r.width*.44,r.height*.43)/1.2;
...
export async function denseStroke(page: Page, axle='rear') {
  ...
  await page.evaluate(axle=>{
    const canvas=document.querySelector<HTMLCanvasElement>(`#drawing-${axle}`)!,r=canvas.getBoundingClientRect(),scale=Math.min(r.width*.44,r.height*.43)/1.2;
