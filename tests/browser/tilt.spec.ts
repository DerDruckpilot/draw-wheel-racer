...
test('vertical axle sliders stay outside the glass and gyro permission, calibration and pause work',async({page,browserName})=>{
  ...
  for(const [width,height] of [[956,440],[667,375]]){
    ...
    const bounds=await page.evaluate(()=>['#wheel-size-rear','#drawing-rear','#drawing-front','#wheel-size-front'].map(q=>document.querySelector(q)!.getBoundingClientRect().toJSON()));
  ...
  const slider=(await page.locator('#wheel-size-rear').boundingBox())!;
  ...
  expect((await snapshot(page)).wheelSizes[0]).toBeLessThan(.52);expect((await snapshot(page)).wheelSizes[1]).toBe(1);
  await page.locator('#wheel-size-rear').focus();await page.locator('#wheel-size-rear').press('End');
  expect((await snapshot(page)).wheelSizes[0]).toBe(1.4);
  ...
  await page.evaluate(()=>{(window as any).__motion=setInterval(()=>window.dispatchEvent(Object.assign(new Event('deviceorientation'),{beta:24,gamma:-24})),40);});
  await expect.poll(async()=>(await snapshot(page)).tuning.weightTarget).toBeLessThan(-.9);
  ...
  await page.evaluate(()=>{clearInterval((window as any).__motion);window.dispatchEvent(Object.assign(new Event('deviceorientation'),{beta:0,gamma:0}));});await expect.poll(async()=>(await snapshot(page)).tuning.weightTarget).toBeGreaterThan(.25);await expect.poll(async()=>(await snapshot(page)).tuning.weightTarget).toBe(0);
