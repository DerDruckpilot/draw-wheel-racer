...
test('vertical axle sliders stay outside the glass and gyro permission, calibration and pause work',async({page,browserName})=>{
  ...
  expect((await snapshot(page)).tuning.flex[0].stiffness).toBeLessThan(.15);expect((await snapshot(page)).tuning.flex[1].stiffness).toBe(1);
  ...
  await page.locator('#pause-button').click();expect((await snapshot(page)).tuning.gyro.weight).toBe(0);expect((await snapshot(page)).tuning.lateral.input).toBe(0);await page.locator('#resume-game').click();
  await page.evaluate(()=>{clearInterval((window as any).__motion);window.dispatchEvent(Object.assign(new Event('deviceorientation'),{beta:0,gamma:0}));});await expect.poll(async()=>(await snapshot(page)).tuning.ballastTarget).toBeLessThan(-.25);await expect.poll(async()=>(await snapshot(page)).tuning.ballastTarget).toBe(0);
  ...
  await page.screenshot({path:'.local/v110-sliders.png'});expect(errors).toEqual([]);
});
...
test('denied sensor permission leaves the touch cockpit and manual steering usable',async({page,browserName})=>{
  ...
  await page.goto('?test=1');await expect(page.locator('#start-button')).toHaveText(/Motor starten/,{timeout:60000});await page.locator('#tune-button').click();await page.locator('#gyro-enable').click();
  ...
  await page.locator('#steering').focus();await page.locator('#steering').press('End');expect((await snapshot(page)).tuning.lateral.input).toBe(1);await page.locator('#close-tune').click();expect((await snapshot(page)).tuning.lateral.input).toBe(0);
