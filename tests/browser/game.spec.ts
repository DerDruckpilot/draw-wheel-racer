...
test('landscape drafts require explicit mounting on each axle, with multi-stroke undo and no presets',async({page})=>{
  ...
  await page.locator('#settings-button').click();await expect(page.locator('.version')).toContainText('1.10.0');await page.locator('#close-modal').click();
  ...
  await page.locator('#finish-home').click();await expect(page.locator('[data-level]')).toHaveCount(22);expect(errors).toEqual([]);
});
