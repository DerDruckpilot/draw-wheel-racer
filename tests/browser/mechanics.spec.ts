...
test('mechanical adventures render in mobile WebKit, expose live tuning, and preserve the drawing cockpit',async({page,browserName})=>{
  ...
  await page.evaluate(()=>{const a=(window as any).__FORMDRIVE__;a.load(17);const w=a.course().waters.find((w:any)=>w.fall);a.inspect(w.fall.x-2,'paddle');a.drive(.5);a.step(240);});
  await page.screenshot({path:'.local/v19-river.png'});expect((await snapshot(page)).player.resets).toBe(0);
  await page.evaluate(()=>{const a=(window as any).__FORMDRIVE__;a.load(18);const m=a.course().mechanisms.find((m:any)=>m.kind==='breakice');a.inspect(m.x,'round');a.drive(0,1);a.step(240);});
  ...
  await page.evaluate(()=>{const a=(window as any).__FORMDRIVE__;a.load(19);a.inspect(a.course().masterRoutes[0].marks[0].x-7,'grip');});
